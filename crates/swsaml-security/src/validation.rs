...
use swsaml_core::constants::{CM_BEARER, NAMEID_ENTITY, NAMEID_PERSISTENT};
use swsaml_core::protocol::response::Response;

use crate::audience::evaluate_audience_restrictions;
use crate::clock::{is_not_before_valid, is_not_on_or_after_valid, is_within_age_limit};
use crate::conditions::{check_one_time_use, check_proxy_restriction};
use crate::config::SecurityConfig;
use crate::destination::verify_destination;
use crate::error::{ValidationCheck, ValidationResult};
use crate::name_id::PersistentIdStore;
use crate::recipient::verify_recipient;
use crate::relay_state::validate_relay_state_content;
use crate::replay::ReplayCache;
use crate::signature::contains_ds_object;
...
impl<'a> AssertionValidator<'a> {
    ...
    fn check_assertion_level(
        ...
        if assertion.has_signature {
            ...
            // is delegated to swsaml-crypto and is expected to be done before
    ...
    fn check_name_id(
        ...
        if self.config.enforce_persistent_id_uniqueness {
            if let Some(ref name_id) = subject.name_id {
                match name_id {
                    swsaml_core::assertion::name_id::NameIdOrEncryptedId::NameId(nid) => {
                    ...
                    swsaml_core::assertion::name_id::NameIdOrEncryptedId::EncryptedId(_) => {
    ...
    fn check_relay_state(&self, relay_state: &str, result: &mut ValidationResult) {
        ...
        if relay_state.len() <= crate::relay_state::MAX_RELAY_STATE_BYTES {
