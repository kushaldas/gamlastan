...
use swsaml_crypto::signer::SamlSigner;
use swsaml_profiles::artifact_resolution;
use swsaml_profiles::logout;
use swsaml_profiles::sso::idp as idp_profile;
use swsaml_profiles::sso::web_browser::ResponseOptions;
...
pub struct AuthnCallbackResult {
    ...
    pub attributes: Vec<swsaml_core::assertion::attribute::Attribute>,
...
pub fn insert_signature_after_element(xml: &str, element_name: &str, sig_template: &str) -> Result<String, SamlActixError> {
    // Find the opening tag for the element
    let search_with_space = format!("<{element_name} ");
    let search_with_close = format!("<{element_name}>");

    let tag_start = xml.find(&search_with_space)
...
pub fn sign_response_xml(
    ...
    if sign_assertions {
        if let Some(assertion_id) = assertion_id {
            ...
            xml = signing_ctx.signer.sign_enveloped(&xml)
...
async fn idp_sso(
    ...
    let request_ref = swsaml_xml::deserialize::parse_saml::<
        swsaml_core::protocol::request::AuthnRequestRef<'_>,
    >(&doc)?;
    ...
    let response_xml = response.to_xml_string().map_err(|e| {
        SamlActixError::Internal(format!("failed to serialize Response: {e}"))
    })?;
    ...
    let relay_state = relay_state_str
        .as_deref()
        .map(RelayState::echo);
...
async fn idp_artifact_resolve(
    ...
    let resolve_ref = swsaml_xml::deserialize::parse_saml::<
        swsaml_core::protocol::artifact::ArtifactResolveRef<'_>,
    >(&doc)?;
    ...
    let soap_body = swsaml_bindings::soap::soap_envelope_wrap(&response_xml, None);
    Ok(HttpResponse::Ok()
...
async fn idp_metadata(
    ...
    use swsaml_core::identifiers::SamlId;
    ...
    use swsaml_metadata::types::key_descriptor::KeyDescriptor;
    ...
    let key_descriptors = if let Some(cert_b64) = &config.signing_cert_b64 {
        let key_info_xml = swsaml_crypto::build_x509_key_info(&[cert_b64]);
    ...
    let idp_sso_desc = IdpSsoDescriptor {
        sso_base: SsoDescriptorBase {
            ...
            name_id_formats: vec![
                swsaml_core::constants::NAMEID_TRANSIENT.to_string(),
                swsaml_core::constants::NAMEID_EMAIL.to_string(),
            ],
        },
    ...
    let final_xml = if let Some(ctx) = &signing_ctx {
        ...
        ctx.signer.sign_enveloped(&xml_with_sig)
